...
pub const FileDescriptorSet = struct {
    file: List(FileDescriptorProto) = .{}, // 1
        ...
        while (true) {
            ...
            switch (key.field_num) {
                1 => {
                    ...
                    std.debug.print("unexpected key {}\n", .{key});
...
pub const FileDescriptorProto = struct {
    ...
    public_dependency: List(i32) = .{}, // 10
    weak_dependency: List(i32) = .{}, // 11
    message_type: List(DescriptorProto) = .{}, // 4
    enum_type: List(EnumDescriptorProto) = .{}, // 5
    service: List(ServiceDescriptorProto) = .{}, // 6
    extension: List(FieldDescriptorProto) = .{}, // 7
    ...
    pub fn serialize(self: FileDescriptorProto, writer: anytype) Error!void {
        ...
        if (self.has(.message_type)) {
            var iter = self.message_type.constIterator(0);
            while (iter.next()) |it| {
        ...
        if (self.has(.enum_type)) {
            var iter = self.enum_type.constIterator(0);
            while (iter.next()) |it| {
        ...
        if (self.has(.service)) {
            var iter = self.service.constIterator(0);
            while (iter.next()) |it| {
        ...
        if (self.has(.extension)) {
            var iter = self.extension.constIterator(0);
            while (iter.next()) |it| {
        ...
        if (self.has(.weak_dependency)) {
            ...
            var countwriter = std.io.countingWriter(std.io.null_writer);
            const cwriter = countwriter.writer();
            var iter = self.weak_dependency.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeVarint128(i32, it.*, cwriter, .int);
            }
            try decoding.writeVarint128(usize, countwriter.bytes_written, writer, .int);
            var iter1 = self.weak_dependency.constIterator(0);
            while (iter1.next()) |it| {
                try decoding.writeVarint128(i32, it.*, writer, .int);
            }
        }
...
pub const ExtensionRangeOptions = struct {
    uninterpreted_option: List(UninterpretedOption) = .{}, // 999
    __fields_present: std.StaticBitSet(1) = std.StaticBitSet(1).initEmpty(),
    ...
    pub fn serialize(self: ExtensionRangeOptions, writer: anytype) Error!void {
        if (self.has(.uninterpreted_option)) {
            var iter = self.uninterpreted_option.constIterator(0);
            while (iter.next()) |it| {
...
pub const DescriptorProto = struct {
    name: []const u8 = "", // 1
    field: List(FieldDescriptorProto) = .{}, // 2
    extension: List(FieldDescriptorProto) = .{}, // 6
    nested_type: List(DescriptorProto) = .{}, // 3
    enum_type: List(EnumDescriptorProto) = .{}, // 4
    extension_range: List(ExtensionRange) = .{}, // 5
    oneof_decl: List(OneofDescriptorProto) = .{}, // 8
    options: ?*MessageOptions = null, // 7
    reserved_range: List(ReservedRange) = .{}, // 9
    reserved_name: List([]const u8) = .{}, // 10
    ...
    pub fn serialize(self: DescriptorProto, writer: anytype) Error!void {
        ...
        if (self.has(.field)) {
            var iter = self.field.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(2, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
        ...
        if (self.has(.nested_type)) {
            var iter = self.nested_type.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(3, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
        ...
        if (self.has(.extension_range)) {
            var iter = self.extension_range.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(5, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
        ...
        if (self.has(.oneof_decl)) {
            var iter = self.oneof_decl.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(8, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
        ...
        if (self.has(.reserved_range)) {
            var iter = self.reserved_range.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(9, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
        ...
        if (self.has(.reserved_name)) {
            var iter = self.reserved_name.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(10, .length_delimited, writer);
                try decoding.writeString(it.*, writer);
            }
...
pub const EnumDescriptorProto = struct {
    name: []const u8 = "", // 1
    value: List(EnumValueDescriptorProto) = .{}, // 2
    options: ?*EnumOptions = null, // 3
    reserved_range: List(EnumReservedRange) = .{}, // 4
    reserved_name: List([]const u8) = .{}, // 5
    ...
    pub fn serialize(self: EnumDescriptorProto, writer: anytype) Error!void {
        ...
        if (self.has(.value)) {
            var iter = self.value.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(2, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
...
pub const ServiceDescriptorProto = struct {
    name: []const u8 = "", // 1
    method: List(MethodDescriptorProto) = .{}, // 2
    ...
    pub fn serialize(self: ServiceDescriptorProto, writer: anytype) Error!void {
        ...
        if (self.has(.method)) {
            var iter = self.method.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(2, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
...
pub const UninterpretedOption = struct {
    ...
    pub fn serialize(self: UninterpretedOption, writer: anytype) Error!void {
        if (self.has(.name)) {
            var iter = self.name.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(2, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
...
pub const SourceCodeInfo = struct {
    ...
    pub const Location = struct {
        ...
        leading_detached_comments: List([]const u8) = .{}, // 6
        __fields_present: std.StaticBitSet(5) = std.StaticBitSet(5).initEmpty(),
        ...
        pub fn serialize(self: Location, writer: anytype) Error!void {
            if (self.has(.path)) {
                ...
                var countwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countwriter.writer();
                var iter = self.path.constIterator(0);
                while (iter.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, cwriter, .int);
                }
                try decoding.writeVarint128(usize, countwriter.bytes_written, writer, .int);
                var iter1 = self.path.constIterator(0);
                while (iter1.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, writer, .int);
                }
            }
            if (self.has(.span)) {
                ...
                var countwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countwriter.writer();
                var iter = self.span.constIterator(0);
                while (iter.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, cwriter, .int);
                }
                try decoding.writeVarint128(usize, countwriter.bytes_written, writer, .int);
                var iter1 = self.span.constIterator(0);
                while (iter1.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, writer, .int);
                }
            }
            ...
            if (self.has(.leading_detached_comments)) {
                var iter = self.leading_detached_comments.constIterator(0);
                while (iter.next()) |it| {
                    try decoding.writeFieldKey(6, .length_delimited, writer);
                    try decoding.writeString(it.*, writer);
                }
    ...
    pub fn serialize(self: SourceCodeInfo, writer: anytype) Error!void {
        if (self.has(.location)) {
            var iter = self.location.constIterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(1, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
...
pub const GeneratedCodeInfo = struct {
    ...
    pub const Annotation = struct {
        ...
        pub fn serialize(self: Annotation, writer: anytype) Error!void {
            if (self.has(.path)) {
                ...
                var countwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countwriter.writer();
                var iter = self.path.iterator(0);
                while (iter.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, cwriter, .int);
                }
                try decoding.writeVarint128(usize, countwriter.bytes_written, writer, .int);
                var iter1 = self.path.iterator(0);
                while (iter1.next()) |it| {
                    try decoding.writeVarint128(i32, it.*, writer, .int);
                }
            }
    ...
    pub fn serialize(self: GeneratedCodeInfo, writer: anytype) Error!void {
        if (self.has(.annotation)) {
            var iter = self.annotation.iterator(0);
            while (iter.next()) |it| {
                try decoding.writeFieldKey(1, .length_delimited, writer);
                var countingwriter = std.io.countingWriter(std.io.null_writer);
                const cwriter = countingwriter.writer();
